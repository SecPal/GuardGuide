...
- Fixed `npm ci` skipping the `@fontsource/inter` install because dependabot duplicated `@rollup/rollup-linux-x64-gnu` into the root `dependencies` block of `package-lock.json` while it was also declared in `optionalDependencies`; this left the lockfile out of sync and caused `vite build` to fail with `Rolldown failed to resolve import "@fontsource/inter/400.css"` on the login bundle. Regenerated the lockfile so the duplicate is removed and a clean install resolves the Inter font assets again.
