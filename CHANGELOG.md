...
- Upgraded the frontend lint toolchain to ESLint 10, replaced the legacy React and import plugins with `@eslint-react/eslint-plugin` and `eslint-plugin-import-x`, excluded Polyscope's `.context` data from lint and test discovery, and added configuration regression coverage so React rules can no longer be silently overwritten. The supported dependency graph now uses `minimatch 10.2.6` with `brace-expansion 5.0.8`, resolving the high-severity unbounded expansion denial-of-service advisory [CVE-2026-14257 / GHSA-mh99-v99m-4gvg](https://github.com/advisories/GHSA-mh99-v99m-4gvg) without a cross-major override.
- Isolated server-generated two-factor QR SVGs in image data URLs instead of injecting them into the DOM as inline HTML, preventing SVG content from becoming an executable markup sink.
- Cleaned up deferred focus and prefetch-cache timers on unmount, replaced unstable array-index React keys, aligned ref names with their purpose, and selected newly created roles directly from the successful Inertia response instead of setting state from an effect.
